...
if ( $_template == '2' ) {
    ...
    $loop = mbstore_woo_query($_orderby, $_row*$_number_desktop , $_cat);
}else{
    $loop = mbstore_woo_query($_orderby, $_limit, $_cat);
}

?>
<div id="<?php echo esc_attr( $_content_id ); ?>" class="prdlist-content">
<?php
if( $loop->have_posts() ) :
    ?>
    <div class="ajaxtab-products product_list grid <?php if ( $_template == '1' ) echo 'owl-carousel'?> <?php echo $_effect; ?>" data-size="<?php echo ($loop->post_count/$_row); ?>">
        <?php
        $i = 0;
        while ( $loop->have_posts() ) : $loop->the_post();
            ...
            wc_get_template( 'vc/grid.php', array('class' => $_class) );
...
<?php
    ...
    echo '<p>'.esc_html__('There are no products matching to show', 'mbstore-shortcodes').'</p>';
endif;
