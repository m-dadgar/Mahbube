<?php
if ( ! class_exists( 'MBStoreCompileSass' ) ) {
	class MBStoreCompileSass {
		function mbstore_getstyle($compile = 2, $scss = array('dir' => '', 'name' => ''), $css = array('dir' => '', 'name' => ''), $format = 'scss_formatter_compressed', $variables = array() ) {
			...
			if($variables) {
				foreach($variables as $propety => $value) {
					...
					$css['name'] .= '-'.strtolower(preg_replace('/\W/i', '', $value)); // If you want color code in file name: 
				}
			}
			if( $compile == 2 || !file_exists(get_template_directory() . '/assets/css/' . $css['name'] . '.css') )
				$this->mbstore_compilescss($scss, $css, $format, $scss_variables);
			return $css['name'] . '.css';
		}
		function mbstore_compilescss($scss, $css, $format, $scss_variables) {
