<?php
class mbstore_Megamenu_Front extends Walker_Nav_Menu { 
    var $columns = 0;
    var $enablemega = 0;
    var $usepostwcode = '';
    var $postwcode = '';
        ...
        if($depth === 0 && $this->enablemega){
            ...
            if ( $this->postwcode !='' && $this->usepostwcode != '' ) {
                ...
                if ($m_wcode->have_posts()) {
                    $this->thepostwcode = do_shortcode('[mbstore_postwcode name="'.$this->postwcode.'"]');
                    if ( $this->thepostwcode != '' ) {
                        $class .= ' has-'.esc_attr($this->usepostwcode).'-content';
                        if ( $this->usepostwcode == 'left' ) {
                            $l_content = '<div class="mega-left-content">'.$this->thepostwcode.'</div>';
                        }
                    }
                }
                wp_reset_postdata();
                ...
                if ($m_wcode->have_posts()) {
                    ...
                    if ( $this->thepostwcode != '' ) {
                        if ( $this->usepostwcode == 'right' ) {
                            $r_b_content = '<div class="mega-right-content">'.$this->thepostwcode.'</div>';
                        }elseif ( $this->usepostwcode == 'bottom' ) {
                            $r_b_content = '<div class="mega-bottom-content">'.$this->thepostwcode.'</div>';
                        }
                    }
                }
                wp_reset_postdata();
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if($depth === 1 && $this->enablemega) {
            ...
            if ( get_post_meta( $item->ID, '_sns_megamenu_item_icon', true) != '' && get_post_meta( $item->ID, '_sns_megamenu_item_useicon', true) != '' ) {
                if ( get_post_meta( $item->ID, '_sns_megamenu_item_useicon', true) == 'font' ) {
                    $item_output .='<span class="icon"><i class="'.get_post_meta( $item->ID, '_sns_megamenu_item_icon', true).'"></i></span>';
               }elseif( get_post_meta( $item->ID, '_sns_megamenu_item_useicon', true) == 'image' ){
                    $item_output .= '<span class="icon"><img class="sns-megamenu-icon-img" src="'.get_post_meta( $item->ID, '_sns_megamenu_item_icon', true).'" alt="'.esc_attr($item->title).'"/></span>';
               }
            }
            $item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
            $item_output .= '</a>';
        ...
        $class_names = ' class="'.esc_attr($li_text_block_class) . esc_attr( $class_names ) . $column_class.'"';
