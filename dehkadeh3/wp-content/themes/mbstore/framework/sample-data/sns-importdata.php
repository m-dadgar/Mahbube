<?php
if ( ! function_exists( 'mbstore_importdata' ) ) {
	function mbstore_importdata(){
		$msg = '';
		...
		if( $datatype=='theme' ){
			...
			mbstore_import_themeoptions($option_data);
		}
		if( $datatype=='slider' ){
			if(!mbstore_import_revslider()){
		...
		if( $datatype=='content' ){
			...
			wp_delete_nav_menu('all-categories');
			wp_delete_nav_menu('customer-care');
			wp_delete_nav_menu('find-in-fast');
		    ...
		    foreach($locations as $locationId => $menuValue){
		        switch($locationId){
		            ...
		            case 'categories_navigation':
		                $menu = get_term_by('name', 'All Categories', 'nav_menu');
		            break;
		            case 'main_categories_navigation':
		...
		if( $datatype=='widget' ){
	        ...
	        //ob_start();
			if ( !mbstore_import_widget($widget_data) ){
		...
		if( $datatype=='media4' ){
			...
			$wp_import_m->fetch_attachments = true;
			// Import media
			$wp_import_m->import(get_template_directory() . '/framework/sample-data/data/media4.xml');
		}
		ob_end_clean();
		...
		elseif($datatype == 'media3'){
			$msg .= 'Import media3 done.';
		}
		elseif($datatype == 'media4'){
			$msg .= 'Import media4 done. finished!';
		}
		die($msg);
	}
}
if ( ! function_exists( 'mbstore_import_themeoptions' ) ){
	function mbstore_import_themeoptions($option){
		$option = json_decode($option,true);
		update_option('mbstore_themeoptions',$option);
	}
}
if(!function_exists('mbstore_import_revslider')){
	function mbstore_import_revslider(){
...
if(!function_exists('mbstore_import_widget')){
	function mbstore_import_widget($import_array){
		...
		foreach ( $sidebars_data as $import_sidebar => $import_widgets ) :
			foreach ( $import_widgets as $import_widget ) :
				...
				if ( isset( $wp_registered_sidebars[$import_sidebar] ) ) :
					...
					$new_widget_name 		= mbstore_widget_name( $title, $index );
...
if(!function_exists('mbstore_widget_name')){
	function mbstore_widget_name($widget_name, $widget_index){
