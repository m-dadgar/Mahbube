(function ($) {
    "use strict";
	// Save current page
	var _current_page, item_template, content_div, _ajax_loading = false;
	if(typeof sns !== 'undefined' && typeof sns.query_vars !== 'undefined'){
	...
	function mbstore_do_ajax($blog_layout){ 
			...
			$('#navigation-ajax').live('click', function(e){
				...
				if( _current_page > -1 && !_ajax_loading){
					...
					data = {
							...
							mbstore_blog_layout: $blog_layout,
							vars: sns.query_vars,
					};
	...
	function mbstore_auto_do_ajax($blog_layout){ 
		if( $('#navigation-ajax').length > 0 ){
	...
	$(document).ready(function($){
		if( $('#navigation-ajax').length > 0 ){
			var $mbstore_blog_layout = $('input[name="hidden_mbstore_blog_layout"]').val();
			if ( $('#navigation-ajax').hasClass('click-load') ) {
				$('#navigation-ajax').on('click', function(){
					...
					mbstore_do_ajax($mbstore_blog_layout);

				});
			}else{
				$(window).scroll(function() {
					var scrollBottom = $(window).scrollTop() + $(window).height();
					var navTop = $('#navigation-ajax').offset().top; //console.log(navTop + ' vs ' + scrollBottom );
					if ( navTop + $('#navigation-ajax').outerHeight() <= scrollBottom ) {
						setTimeout(function(){
							...
							mbstore_auto_do_ajax($mbstore_blog_layout);
						}, 800);
					}
				});
			}
