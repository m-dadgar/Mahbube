...
if ( ! class_exists( 'YITH_WC_Product_Countdown' ) ) {
	...
	class YITH_WC_Product_Countdown {
		...
		public function write_tab_options() {
			...
			$sale_price_dates_from = ( $date = yit_get_prop( $product, '_ywpc_sale_price_dates_from', true ) ) ? date_i18n( 'Y-m-d', $date ) : '';
			$sale_price_dates_to   = ( $date = yit_get_prop( $product, '_ywpc_sale_price_dates_to', true ) ) ? date_i18n( 'Y-m-d', $date ) : '';
		...
		public function frontend_scripts() {
			...
			wp_enqueue_script( 'ywpc-footer', YWPC_ASSETS_URL . '/js/ywpc-footer' . $suffix . '.js', array( 'jquery' ), false, true );
