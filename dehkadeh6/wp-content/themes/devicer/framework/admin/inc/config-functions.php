...
if (!function_exists('cmsmasters_fonts_list')) {
	function cmsmasters_fonts_list() {
		...
		if (isset($cmsmasters_option['devicer' . '_google_web_fonts']) && is_array($cmsmasters_option['devicer' . '_google_web_fonts'])) {
			$google_web_fonts_array = $cmsmasters_option['devicer' . '_google_web_fonts'];
		...
		$fonts = array( 
			'' => esc_html__('None', 'devicer'), 
			'local' => $local_fonts, 
			'web' => $google_web_fonts 
		);
		
		
		return apply_filters('devicer_google_fonts_list_filter', $fonts);
	}
}
...
if (!function_exists('cmsmasters_text_transform_list')) {
	function cmsmasters_text_transform_list() {
		$list = array( 
			...
			'uppercase' => esc_html__('uppercase', 'devicer'), 
			'lowercase' => esc_html__('lowercase', 'devicer'), 
			'capitalize' => esc_html__('capitalize', 'devicer'), 
		);
...
if (!function_exists('cmsmasters_image_thumbnail_list')) {
	function cmsmasters_image_thumbnail_list() {
		$list = devicer_get_image_thumbnail_list();
...
if (!function_exists('cmsmasters_color_schemes_list')) {
	function cmsmasters_color_schemes_list() {
		$list = devicer_all_color_schemes_list();
