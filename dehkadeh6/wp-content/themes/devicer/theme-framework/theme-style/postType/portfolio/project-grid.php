...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && devicer_project_check_exc_cont()) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		echo '<div class="cmsmasters_project_images_wrap">';
			...
			devicer_thumb_rollover(get_the_ID(), $project_thumb_size, false, $rollover, false, false, false, false, false, $project_thumb_high, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
			
			
			if ($comments || $likes) {
				echo '<footer class="cmsmasters_project_footer entry-meta">';
					
					$comments ? devicer_get_project_comments('page') : '';
					
					$likes ? devicer_get_project_likes('page') : '';
		...
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">';
				...
				$title ? devicer_project_heading(get_the_ID(), 'h2', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';
				
				
				if ($categories) {
					echo '<div class="cmsmasters_project_cont_info entry-meta">';
						
						devicer_get_project_category(get_the_ID(), 'pj-categs', 'page');
						
					echo '</div>';
				}
				
				
				$excerpt ? devicer_project_exc_cont() : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				devicer_project_heading(get_the_ID(), 'h6');
			echo '</div>';
		}
