...
function devicer_project_heading($cmsmasters_id, $tag = 'h1', $link_redirect = false, $link_url = false, $link_target = false, $show = true) { 
...
function devicer_project_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
...
function devicer_project_exc_cont($show = true) {
	$out = cmsmasters_divpdel('<div class="cmsmasters_project_content entry-content">' . "\n" . 
		wpautop(devicer_excerpt(20, false)) .
	'</div>' . "\n");
...
function devicer_project_check_exc_cont() {
	$exc = devicer_project_exc_cont(false);
...
function devicer_get_project_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			$out = '<span class="cmsmasters_project_category">' . 
				devicer_get_the_category_list($cmsmasters_id, $taxonomy, ', ') . 
			'</span>';
		} elseif ($template_type == 'post') {
			$cmsmasters_option = devicer_get_global_options();
			
			
			if ($cmsmasters_option['devicer' . '_portfolio_project_cat']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Categories', 'devicer') . ':' . '</div>' . 
...
function devicer_get_project_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['devicer' . '_portfolio_project_like']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Likes', 'devicer') . ':' . '</div>' . 
...
function devicer_get_project_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = devicer_get_comments('cmsmasters_project_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = devicer_get_global_options();
			
			if ($cmsmasters_option['devicer' . '_portfolio_project_comment'] && comments_open()) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Comments', 'devicer') . ':' . '</div>' . 
					'<div class="project_details_item_desc details_item_desc_comments">' . 
						devicer_get_comments('cmsmasters_project_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function devicer_get_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['devicer' . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'devicer') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function devicer_get_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_project_author">' . 
			esc_html__('By', 'devicer') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'devicer') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($template_type == 'post') {
		$cmsmasters_option = devicer_get_global_options();
		...
		if ($cmsmasters_option['devicer' . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'devicer') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
					'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'devicer') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
				'</div>' . 
			'</div>';
...
function devicer_get_project_tags($cmsmasters_id, $taxonomy, $show = true) {
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		$cmsmasters_option = devicer_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['devicer' . '_portfolio_project_tag']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Tags', 'devicer') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function devicer_get_project_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) || 
			!empty($features[0][1])
...
function devicer_project_link($link_text, $link_url, $link_target, $show = true) {
	$cmsmasters_option = devicer_get_global_options();
	$out = '';
	
	if ( 
		$cmsmasters_option['devicer' . '_portfolio_project_link'] && 
		$link_text != '' && 
		$link_url != '' 
	) {
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Project Link', 'devicer') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
