...
echo '<div class="images cmsmasters_product_images">';
	if (method_exists($product, 'get_available_variations')) {
		...
		foreach ($cmsmasters_product_variable_items as $cmsmasters_product_variable_item) {
			if ($cmsmasters_product_variable_item['image']['full_src'] != '' && $post_thumbnail_id != $cmsmasters_product_variable_item['image_id']) {
	...
	if ($product->get_image_id()) {
		...
		$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'devicer' ) );
 		$html .= '</div>';
	}
